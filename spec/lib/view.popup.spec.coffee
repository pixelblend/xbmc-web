describe 'Popup', () ->
  ...
  describe 'with a player', () ->
    ...
    it 'does not render without a playlist', () ->
      expect(@canvas.find('#play-details').text()).toBe('stopped')
    
    describe 'on playlist update', () ->
      beforeEach () ->
        @player.playlist = new AudioPlaylist @track
        @player.trigger('changed:playlist')
        
      it 'renders an updated view', () ->
